//! `ryu-vault` — the Identity Vault primitive: crypto-sealed per-domain agent
//! connections (Unit 0, #518), extracted from `apps/core` as an in-process
//! library crate with a function-call hot path (no IPC).
//!
...
//!
//! ## Placement (this crate vs kernel)
//!
//! Per `CLAUDE.md` §1, the [`IdentityStore`], the encryption seam
//! ([`ryu_crypto::FieldCipher`]), the [`source`] capture/rotation backends, and
//! the [`health`] staleness sweep all decide *what runs* over stored secrets, so
//! they are the swappable **primitive** and live here. What stays kernel-side in
//! `apps/core` is the identity *governance* — the `identity.read` grant + audit
//! read chokepoint (`crate::identity::read_credential`, #523), the tool-call-time
//! consult, and the elicitation seam — because they decide *what is
//! allowed/measured* and depend on the Gateway/tool plane. This crate is
//! gateway-agnostic and has ZERO dependency on `apps/core`; the one kernel
//! coupling (the `~/.ryu` data dir) is inverted as the [`IdentityStore::open`]
//! constructor parameter (a single construction site — cf. the `CryptoHost`
//! global-host precedent, unneeded here because there is no lazy global).
//!
//! ## Secret hygiene
//!
...
/// Seed the process-global crypto host + master key for tests. Installs a trivial
/// in-crate [`ryu_crypto::CryptoHost`] (production installs Core's `CoreCryptoHost`
/// at boot) so `global_cipher()` resolves without an OS keychain, and sets a
/// deterministic `RYU_MASTER_KEY` that short-circuits the custody ladder. Both
/// steps are idempotent; call from any test that seals/opens at-rest state.
///
/// Exposed under the `test-support` feature (not just `cfg(test)`) so Core's own
/// identity-governance tests — which live in `apps/core` and drive this crate's
/// [`IdentityStore`] — can seal/open at-rest state exactly as production does.
#[cfg(any(test, feature = "test-support"))]
pub(crate) fn ensure_test_cipher() {
    struct TestHost;
    impl ryu_crypto::CryptoHost for TestHost {
        fn keyring_account_suffix(&self) -> String {
            String::new()
        }
        fn ryu_dir(&self) -> std::path::PathBuf {
            std::env::temp_dir().join("ryu-vault-test")
        }
    }
    ryu_crypto::set_global_host(std::sync::Arc::new(TestHost));

    // Precomputed base64 of `[7u8; 32]` — a fixed test key, inlined so this helper
    // (and the `test-support` build) needs no base64 dependency.
    ...
    unsafe {
        std::env::set_var("RYU_MASTER_KEY", TEST_MASTER_KEY);
    }
}
