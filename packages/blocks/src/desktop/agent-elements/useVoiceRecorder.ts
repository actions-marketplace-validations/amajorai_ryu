import { downsample, encodeWav } from "../../companion/audio-pcm.ts";
...
import { getDefaultMicId } from "./voice-devices.ts";

export type RecorderState = "idle" | "recording" | "transcribing";

/**
 * Length of the amplitude history (each entry is one moment in time). The
 * composer's recording state renders a wide, full-width waveform, so keep enough
 * history to feed a dense bar layout; the `Wave` component buckets it down to
 * whatever bar count it draws.
 */
const BAR_COUNT = 64;
