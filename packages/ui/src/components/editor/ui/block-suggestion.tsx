...
import {
	...
	AvatarFallback,
	AvatarImage,
} from "@ryu/ui/components/editor/ui/avatar.tsx";
import { Button } from "@ryu/ui/components/editor/ui/button.tsx";
import {
	BLOCK_SUGGESTION_TOKEN,
	type ResolvedSuggestion,
} from "@ryu/ui/lib/block-discussion-index.ts";
import { CheckIcon, XIcon } from "lucide-react";
import { useEditorPlugin, usePluginOption } from "platejs/react";
import { Fragment, useState } from "react";
import { Comment, CommentCreateForm, formatCommentDate } from "./comment.tsx";
...
export function BlockSuggestionCard({
	...
	const getRemoveSummaryItems = (text: string) => {
		...
		if (items.includes("Table")) {
			return ["Table"];
		}
		if (items.includes("Code Block")) {
			return ["Code Block"];
		}
