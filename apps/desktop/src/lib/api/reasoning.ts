// apps/desktop/src/lib/api/reasoning.ts
//
// Typed client for the Automated Reasoning API (`/api/reasoning/*`) — the
// `public_mount` Core serves on behalf of the `ryu-reasoning` sidecar. Field names
...
//
// The same reason `social.ts` is: reasoning has no desktop page. Its whole surface is
...
export interface ReasoningRequestInput {
	...
	/** Path RELATIVE to `/api/reasoning`, leading slash included, query and all. */
	path: string;
}

/**
 * Validate a frame-supplied sub-path and return the full node path, or `null` when it
 * is not a path under the reasoning mount.
 *
...
/**
 * Forward one companion call to the reasoning sidecar through Core.
 *
 ...
 */
export async function reasoningRequest(
	target: ApiTarget,
