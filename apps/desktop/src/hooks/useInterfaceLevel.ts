...
//
// React binding for THE interface-mode store (`@/src/lib/interface-level.ts`).
// The store itself is pure so the switch's rules can be imported — and tested —
// without React; this file is only the `useSyncExternalStore` wiring, in the
// same shape as useAgentRowStyle, so the account menu's switch, the composer and
