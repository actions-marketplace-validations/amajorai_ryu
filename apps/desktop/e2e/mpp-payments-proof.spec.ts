...
const challengeHash = 'sha256:91a75a13df08c63c5d58c3368320e520b79de900905b8f26b53e633f8e9c2a9e'
...
async function installMppBridge(
  ...
  await page.addInitScript(({ initialReceipt }) => {
    ...
    const makePayment = (transport: 'http' | 'mcp') => ({
      ...
      payment: {
        ...
        challenge: {
          amountAtomic: '250000',
          chainId: 42431,
          challengeHash: 'sha256:91a75a13df08c63c5d58c3368320e520b79de900905b8f26b53e633f8e9c2a9e',
          challengeId: 'challenge-proof-001',
          currency: '0x20c0000000000000000000000000000000000000',
          decimals: 6,
        ...
        request: transport === 'mcp'
          ...
          : { method: 'GET', transport, url: 'https://mpp.dev/api/ping/paid' },
    ...
    const bridgeWindow = window as typeof window & {
      ...
      ryu?: {
        app: {
          request(input: { body?: unknown; method?: string; path: string }): Promise<unknown>
        }
        context: null
        shell: {
    ...
    bridgeWindow.ryu = {
      app: {
        async request(input) {
          ...
          if (method === 'GET' && input.path === '/status') return status
              ...
              services: [
                {
                  ...
                  endpoints: [
                    { method: 'GET', path: '/api/ping/paid', price: '0.25 pathUSD' },
                  ],
          ...
          if (method === 'GET' && input.path === '/receipts') return { receipts }
          ...
          if (method === 'POST' && input.path === '/payments/pay') {
            ...
            if (receipts.length === 0) receipts.push(settledReceipt)
            return {
          ...
          throw new Error(`Unexpected MPP proof request: ${method} ${input.path}`)
        },
      },
      context: null,
      shell: {
...
async function proofCalls(page: Page): Promise<Array<{ method: string; path: string }>> {
...
test('reviews an exact HTTP charge and settles to a safe receipt', async ({
  ...
  await expect(page.getByText('Payment verified and receipt saved.')).toBeVisible()
  await expect(page.getByLabel('Receipt detail')).toContainText('Payment settled')
  await expect(page.getByLabel('Receipt detail')).toContainText('0.25 pathUSD')
  await expect(page.getByLabel('Receipt detail')).toContainText('receipt-proof-001')
  await expect(page.getByLabel('Payment status')).toContainText('0.25 / 5')
  await expect(page.getByRole('tab', { name: 'Services' })).toHaveAttribute('aria-selected', 'false')
  ...
  const observedRequests = observedCalls.map(({ method, path }) => `${method} ${path}`)
  ...
  expect(observedRequests.filter((request) => request === 'POST /payments/prepare')).toHaveLength(1)
  expect(observedRequests.filter((request) => request === 'POST /payments/pay')).toHaveLength(1)
  ...
  await writeFile(
    ...
    `${JSON.stringify({
      ...
      calls: safeCalls,
      result: consoleErrors.length === 0 && pageErrors.length === 0 && failedRequests.length === 0
        ? 'verified'
        : 'failed',
    }, null, 2)}\n`,
  )
...
test('surfaces MCP purpose and rejects without spending', async ({ page }) => {
  ...
  await expect.poll(async () => (await proofCalls(page)).some(({ path }) => path === '/payments/pay')).toBe(false)
