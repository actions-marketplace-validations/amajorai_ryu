...
async function handle(req, res) {
	...
	if (path === HEALTH_PATH) {
		...
		if (activated) {
			return send(
				...
				200,
				undefined,
				JSON.stringify({ ok: true, plugin: PLUGIN_ID })
			);
		}
		return send(
			res,
			503,
			undefined,
			JSON.stringify({ ok: false, activating: true })
		);
	}
	...
	if (!requestHandler) {
		return send(
			...
			404,
			undefined,
			JSON.stringify({ error: "no request handler registered" })
		);
	}
...
async function main() {
	...
	try {
		...
		log("failed to import entry module", {
			level: "error",
			error: String(err),
			entry: ENTRY,
		});
