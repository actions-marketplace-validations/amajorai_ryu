//! Built-in orchestration discovery tool (`orchestrator.discover_agents`).
//!
//! Delegation itself already exists: [`super::delegate`]'s `delegate.fanout`
...
//!   1. `orchestrator.discover_agents` — find the right specialist by description.
//!   2. `delegate.fanout` (with that `agent_id`) — hand it the subtask.
//!
...
/// not contain `.` (the tool-id separator).
...
pub fn tools() -> Vec<RegistryTool> {
    vec![RegistryTool {
        id: format!("{SERVER_NAME}.discover_agents"),
        ...
        description: Some(
            "List the other agents available to delegate to, with each one's id, name, and \
             ...
             After choosing one, hand it the subtask with the delegate.fanout tool, passing the \
...
mod tests {
    ...
    fn tool_has_stable_id_and_server() {
        ...
        assert_eq!(tools[0].id, "orchestrator.discover_agents");
        assert_eq!(tools[0].server, SERVER_NAME);
        assert!(!tools[0].name.contains('.'));
